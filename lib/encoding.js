...
encoding.ZERO_HASH = Buffer.alloc(32, 0x00);
...
encoding.MAX_HASH = Buffer.alloc(32, 0xff);
...
/**
 * Serialize number as a u64le.
 * @param {Number} num
 * @returns {Buffer}
 */

encoding.u64 = function u64(num) {
  const data = Buffer.allocUnsafe(8);
  encoding.writeU64(data, num, 0);
  return data;
};

/**
 * Serialize number as a u64be.
 * @param {Number} num
 * @returns {Buffer}
 */

encoding.u64be = function u64be(num) {
  const data = Buffer.allocUnsafe(8);
  encoding.writeU64BE(data, num, 0);
  return data;
};
